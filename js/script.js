// Constantes para peso
const COIN_WEIGHT = 0.02; // Peso de cada moeda em kg (50 moedas = 1 kg)

// Funções para a ficha de personagem
function calculateModifier(attributeValue) {
    return Math.floor((attributeValue - 10) / 2);
}

// Calcula a capacidade de carga com base na Força
function calculateCarryingCapacity(strength) {
    return strength * 7.5; // Em D&D 5E, a capacidade de carga é Força × 15 (em libras), convertido para kg
}

// Atualiza a capacidade de carga quando a Força muda
...
// Adiciona uma nova linha na tabela de equipamento
...
// Remove uma linha da tabela de equipamento
...
// Atualiza o peso total de uma linha específica
...
// Calcula o peso total das moedas
function calculateCoinWeight() {
    ...
    const weight = totalCoins * COIN_WEIGHT;
    
    return weight;
}

// Atualiza o peso das moedas quando houver mudança
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Adiciona listener para atualizar capacidade de carga quando a Força mudar
    const strengthInput = document.getElementById('strength');
    if (strengthInput) {
        strengthInput.addEventListener('change', updateCarryingCapacity);
        strengthInput.addEventListener('input', updateCarryingCapacity);
    }

    // Adiciona primeira linha na tabela de equipamento
    addEquipmentRow();
});
...
// Atualiza a barra de progresso de XP e informações relacionadas
function updateXPProgress() {
    // Obtém o XP atual e calcula o nível
    const currentXP = parseInt(document.getElementById('experiencePoints').value) || 0;
    const calculatedLevel = calculateLevel(currentXP);
    
    // Atualiza o nível e o bônus de proficiência
    ...
    if (oldLevel !== calculatedLevel) {
        ...
        // Notifica sobre a mudança de nível apenas se não for a primeira carga
        if (oldLevel > 0 && oldLevel < calculatedLevel) {
            alert(`Parabéns! Você alcançou o nível ${calculatedLevel}!`);
        }
    }

    // Calcula os valores para a barra de progresso
    ...
    // Atualiza o texto de XP necessário para o próximo nível
    ...
    // Atualiza os modificadores que dependem do bônus de proficiência
    updateModifiers();
}
...
function saveCharacter() {
    ...
    // Salva a imagem do personagem e suas transformações
    const characterImage = document.getElementById('characterImage');
    if (characterImage && characterImage.src) {
        // Verifica se não é a imagem SVG padrão
        if (!characterImage.src.includes('Sem Imagem')) {
            // Salva a imagem
            characterData.characterImage = characterImage.src;
            localStorage.setItem('characterImage', characterImage.src);
            
            // Salva as transformações da imagem
            const imageTransform = {
                zoom: currentZoom,
                rotation: currentRotation
            };
            characterData.imageTransform = imageTransform;
            localStorage.setItem('imageTransform', JSON.stringify(imageTransform));
        } else {
            // Remove os dados de imagem salvos se estiver usando a imagem padrão
...
function loadCharacter() {
    ...
    // Carrega a imagem e suas transformações
    loadCharacterImage(true); // force=true para forçar o carregamento da imagem salva
...
function exportToJson() {
    ...
    // Adiciona as transformações atuais da imagem
    characterData.imageTransform = {
        zoom: currentZoom,
        rotation: currentRotation
    };
        ...
        reader.onload = function(e) {
            try {
                ...
                if (characterData.characterImage) {
                    const characterImage = document.getElementById('characterImage');
                    if (characterImage) {
                        characterImage.src = characterData.characterImage;
                        
                        // Salva a imagem no localStorage também
                        localStorage.setItem('characterImage', characterData.characterImage);
                    }
                }

                // Restaura as transformações da imagem
                if (characterData.imageTransform) {
                    currentZoom = characterData.imageTransform.zoom || 1;
                    currentRotation = characterData.imageTransform.rotation || 0;
...
// Função para inicializar a imagem do personagem
function loadCharacterImage(force = false) {
    ...
    if (savedImage && (force || characterImage.src.includes('Clique para adicionar imagem'))) {
...
document.addEventListener('DOMContentLoaded', function() {
    // Adiciona listeners para os atributos
    const attributes = ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'];
    attributes.forEach(attr => {
    ...
    // Adiciona listeners para as perícias
    ...
    // Inicializa o XP e nível
    updateXPProgress();

    // Inicializa a barra de XP
    updateXPProgress();

    // Inicializa a imagem do personagem
    initializeCharacterImage();
});

/******************************************************* */

// Funções para gerenciamento da imagem do personagem
let currentZoom = 1;
...
function handleImageUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        // Guarda as transformações atuais se houver uma imagem anterior
        const oldZoom = currentZoom;
        const oldRotation = currentRotation;
        
        image.src = e.target.result;
        
        // Aplica as transformações anteriores à nova imagem
        currentZoom = oldZoom;
        currentRotation = oldRotation;
        
        // Aguarda o carregamento da imagem para aplicar as transformações
        image.onload = function() {
            updateImageTransform();
        };
        
        // Salva a imagem nos dados do personagem
        saveCharacterImage(e.target.result);
    };
...
// Função para salvar a imagem junto com os dados do personagem
function saveCharacterImage(imageData) {
    localStorage.setItem('characterImage', imageData);
}

// Função para carregar a imagem quando carregar o personagem
